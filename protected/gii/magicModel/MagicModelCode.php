...
class MagicModelCode extends CCodeModel
{
	...
	public $baseClass='EActiveRecord';
	...
	public function generateBeforeSave($columns)
	{
        $genCalls = '';
		foreach ($columns as $name=>$column) {
			if ( (stripos($name, 'dt_') !== false) or (stripos($name, 'tm_') !== false) or (stripos($name, 'dttm_') !== false) ) {
				$genCalls .= "if (!empty(\$this->{$name}))\n\t\t\t";
	...
	{
		{$genCalls}return parent::beforeSave();
	}\n";
		}
	}

    public function generateAfterFind($columns)
    {
        $genCalls = '';
        foreach ($columns as $name=>$column) {
            if ( strpos($name, 'dt_') === 0 ) {
                $genCalls .= "\n\t\t\t\$this->{$name} = (\$this->{$name} !== '0000-00-00' ) ? date('d-m-Y', strtotime(\$this->{$name})) : '';";
                continue;
            }
            if ( strpos($name, 'tm_') === 0 ) {
                $genCalls .= "\n\t\t\t\$this->{$name} = (\$this->{$name} !== '00:00:00' ) ? date('H:i', strtotime(\$this->{$name})) : '';";
                continue;
            }
            if ( strpos($name, 'dttm_') === 0 ) {
                $genCalls .= "\n\t\t\t\$this->{$name} = (\$this->{$name} !== '0000-00-00 00:00:00' ) ? date('d-m-Y H:i', strtotime(\$this->{$name})) : '';";
                continue;
            }
        }
        if ( !empty($genCalls) ) {
            return "\tpublic function afterFind()
	{
		parent::afterFind();
		if ( in_array(\$this->scenario, array('insert', 'update')) ) { {$genCalls}
		}
	}\n";
        }
    }
